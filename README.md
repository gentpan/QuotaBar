...
- Quota Run: a new Quota Run page in Settings. Without an account, every reading goes into a local run ledger (60 days) showing windows in progress and personal bests — fastest to 100%, time to 50% and 90%, highest peak — and a best can be copied or saved as a share image. After joining (username, display name, region, and ticking the list of what is uploaded), this Mac creates a signing key in the Secure Enclave and, as the ranked device, signs and uploads quota readings and per-minute token counts to quota.run; the server computes results and ranks, shown at quota.run and on a public profile at quota.run/@username. Edit a profile and up to 12 projects, add another Mac with a pairing code, change the ranked device once every 7 days, and leave with Delete my data at any time. Credentials, tokens, prompts, code, file paths and plain emails are never uploaded, and nothing at all is uploaded without joining. The About page's list of connections includes Quota Run.
