...
- Prepaid providers no longer show an empty bar. DeepSeek, Moonshot API, OpenRouter and Xiaomi MiMo cards show each currency's balance in large figures, split into paid and granted when there is a grant and in red when it is too low for API calls. Below it is the spend today, this week or this month, with cost, requests and tokens by API key or by model; the top three are listed and the rest sit under the arrow, hovering a key shows its models, and clicking one opens it: spend and requests today, this week and this month side by side, a bar for each day of the month (hover for the day's cost, requests and tokens), and its models in the chosen period, with All Keys to go back. A deleted key is listed, marked, while it has usage. The dock's hover card shows the same. A Big figure or Gauge desktop card on a prepaid provider becomes a balance card — the balance in large figures, spend today, this week and this month, and on the large card the three keys that spent most this month — and the Provider grid and Closest first cards show the balance without a bar. The notch island and the menu bar's right-click menu show the balance instead of a dash. A copied image shows this month's spend by model, without key names. MiMo's monthly Token Plan keeps its bar.
- DeepSeek can show each API key's and each model's usage: paste the console's sign-in token in the credential field instead of an API key (signed in to platform.deepseek.com, DevTools → Application → Local Storage → the value of userToken). The card then reads the balance, this month's spend, and cost, requests and tokens by key and by model. With an API key it shows the balance and says how to see more. An expired token asks to be pasted again.
...
- OpenRouter with a provisioning key lists every key on the account with its spend today, this week and this month; with an ordinary API key it shows the credits and that key's own spend.
