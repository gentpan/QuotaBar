...
- Prepaid providers no longer show an empty bar. DeepSeek, Moonshot API, OpenRouter and Xiaomi MiMo cards show each currency's balance in large figures, split into paid and granted when there is a grant and in red when it is too low for API calls. Below it is recent usage for Today, 7 days, 30 days or All: the spend (with requests and tokens where the provider counts them) and a chart of bars or a line — hours for today, days for 7 and 30 days, months for all — with the style switched at the top right and remembered, and each bar's figures on hover. Under the chart, usage by API key or by model; the top three are listed and the rest sit under the arrow, and clicking a key opens it: spend and requests today, over 7 and over 30 days side by side, its last 30 days as a chart, and its models in the chosen period, with All Keys to go back. A deleted key is listed, marked, while it has usage. The dock's hover card shows the same. A Big figure or Gauge desktop card on a prepaid provider becomes a balance card — the balance in large figures, spend today, over 7 and over 30 days, and on the large card the 30-day chart — and the Provider grid and Closest first cards show the balance without a bar. The notch island and the menu bar's right-click menu show the balance instead of a dash. A copied image shows 30 days of spend, the chart and usage by model, without key names. MiMo's monthly Token Plan keeps its bar.
- Recent usage with an API key alone: DeepSeek's, Moonshot API's and Xiaomi MiMo's APIs answer with the balance only, so QuotaBar keeps each balance it reads and works out the spend today, over 7 and 30 days and in all from how it fell, charted like the rest; the card says "about" and since when it has been keeping count. A rise is taken as a top-up and not counted.
- DeepSeek shows exact usage and every API key's and model's: Settings → Providers → DeepSeek has Sign in in a browser…, which opens the DeepSeek platform console to sign in to and keeps its sign-in in place of the API key. The card then reads the platform's own figures: the lifetime spend, the last 30 days by day and today by the hour, cost, requests and tokens by key — under the names given to them on DeepSeek — and by model, and the spend month by month for All (a month that is over is read once). With an API key it shows the balance and the estimate, and says how to see more; an expired sign-in asks to sign in again. The platform's userToken can also be pasted by hand.
