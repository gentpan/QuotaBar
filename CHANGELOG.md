...
- 充值型服务商不再显示空进度条：DeepSeek、Kimi 开放平台、OpenRouter、小米 MiMo 的卡片改为大字显示各币种余额（有赠送时分列充值和赠送，余额不足以调用 API 时标红）。下面是最近用量，可在「今日」「7 天」「30 天」「全部」之间切换，显示花费（有数据时还有请求次数和 token），并画成柱状图或折线图：今日按小时、7 天和 30 天按天、全部按月，右上角切换图表样式并记住，悬停某一柱看这一小时、这一天或这个月的明细。图表下可切换按 API Key 或按模型查看，默认列出用得最多的 3 个，其余收进展开箭头；点击某个 Key 切到它的详情：今日、7 天、30 天三栏花费和请求次数，最近 30 天每天的用量图，以及所选时间段用到的模型，点「全部 Key」返回；已删除的 Key 在有用量时也列出并注明。停靠条悬停弹出的详情卡同样显示余额区。桌面卡片「大数字」「环形仪表」选中充值型服务商时改为余额卡片（大字余额，今日、7 天、30 天消费，大尺寸再画出 30 天用量图），「服务商网格」「紧迫排行」里显示余额金额、不画进度条。刘海岛和菜单栏右键菜单里显示余额金额，不再是「—」。复制为图片时显示 30 天的消费、用量图和按模型的用量，不含 Key 名称。小米 MiMo 的 Token Plan 月额度仍按进度条显示。
- 只填 API Key 也能看到最近用量：DeepSeek、Kimi 开放平台、小米 MiMo 的接口只返回余额，QuotaBar 会记下每次读到的余额，按余额的减少估算今日、7 天、30 天和全部的消费并画图，卡片上标明「约消费」和开始记录的时间；余额上涨视为充值，不计入。
- DeepSeek 可以看精确用量和每个 API Key、每个模型的明细：设置 → 服务商 → DeepSeek 新增「浏览器登录…」，在弹出的窗口里登录 DeepSeek 开放平台，QuotaBar 自动取得登录凭证并替换掉 API Key，之后读到开放平台自己的数据：累计消费，最近 30 天按天、今天按小时的花费，按 Key（显示你在 DeepSeek 设置的备注名）和按模型的花费、请求次数和 token，以及按月的全部消费走势（已经过去的月份只读一次）。只填 API Key 时显示余额和估算用量，并提示如何看到明细；登录过期时提示重新登录。也可以手动粘贴开放平台的 userToken。
