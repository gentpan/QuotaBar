...
- 充值型服务商不再显示空进度条：DeepSeek、Kimi 开放平台、OpenRouter、小米 MiMo 的卡片改为大字显示各币种余额（有赠送时分列充值和赠送，余额不足以调用 API 时标红），下面是今日、本周、本月的消费，可切换按 API Key 或按模型查看花费、请求次数和 token，默认列出用得最多的 3 个，其余收进展开箭头，悬停某个 Key 可看它用了哪些模型，点击则切到这个 Key 的详情：今日、本周、本月三栏花费和请求次数，本月每天的花费柱状图（悬停看当天金额、请求次数和 token），以及所选时间段用到的模型，点「全部 Key」返回；已删除的 Key 在有用量时也列出并注明。停靠条悬停弹出的详情卡同样显示余额区。桌面卡片「大数字」「环形仪表」选中充值型服务商时改为余额卡片（大字余额、今日本周本月消费，大尺寸再列出本月花费最多的 3 个 Key），「服务商网格」「紧迫排行」里显示余额金额、不画进度条。刘海岛和菜单栏右键菜单里显示余额金额，不再是「—」。复制为图片时只显示本月消费和按模型的用量，不含 Key 名称。小米 MiMo 的 Token Plan 月额度仍按进度条显示。
- DeepSeek 可以看每个 API Key 和每个模型的用量：凭据框里改填控制台登录令牌（登录 platform.deepseek.com 后，开发者工具 → 应用 → 本地存储 → userToken 的值），就能读到余额、本月消费，以及按 Key、按模型的花费、请求次数和 token；只填 API Key 时显示余额，并提示如何看到明细。令牌失效时提示重新填写。
...
- OpenRouter 填管理密钥（Provisioning Key）时列出账户下所有 Key 今日、本周、本月的花费；填普通 API Key 时显示余额和这个 Key 自己的花费。
