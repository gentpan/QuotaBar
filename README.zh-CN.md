...
- 项目公开到 Quota Run：登录 Quota Run 后，每个项目有「公开到 quota.run」开关，默认全部不公开，还可以改公开名称，公开后可以一键打开项目页。这台 Mac 会按天上传各项目、各工具、各编程方式、各模型的 token 数：公开的项目带名称和仓库地址，不公开的合并成「其他项目」、不带名字；路径和会话内容永远不上传。花费由 quota.run 按公开价目统一计算。改动开关或名称后会把全部日子重传一遍，关掉的项目立刻从网站上撤下。
