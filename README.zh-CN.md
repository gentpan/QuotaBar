...
- Quota Run 额度竞速：设置里新增「Quota Run」页。不注册也能用：每次读数自动记进本机成绩账本（保留 60 天），显示进行中的额度窗口和个人最佳（最快用满、用到 50%/90% 的时间、最高峰值），成绩可以复制或保存为分享图片。主动加入后（用户名、显示名称、地区，看过并勾选上传清单）：这台 Mac 生成存在安全芯片里的签名密钥，作为计分设备把额度读数和每分钟 token 数签名上传到 quota.bar，成绩和排名由服务器计算，排行榜在 quota.bar/leaderboard，个人主页在 quota.bar/@用户名；可编辑资料和最多 12 个项目，用配对码添加另一台 Mac，更换计分设备有 7 天冷却，随时「退出 Quota Run 并删除我的数据」。凭据、令牌、提示词、代码、文件路径和明文邮箱永远不上传；不加入就什么都不上传。关于页的联网清单相应补上 Quota Run。
