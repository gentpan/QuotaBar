...
最新版本 **0.5.4**（2026-09-15） · [完整更新日志](CHANGELOG.md)

<details open>
<summary><b>2026-09-15</b> · 0.5.4 · 新增 8 · 修复 4</summary>

**新增**

- 右键「复制为图片」时，服务商卡片上的登录账号默认显示为马赛克，图片里不再出现邮箱地址；马赛克是固定图案，不透露账号内容和长度。可在设置 → 通用 → 隐私里关闭「复制图片时遮挡账号」，下拉面板里的账号照常显示。
- 服务商卡片的右键菜单新增「隐藏额度」，可以把用不到的额度彻底隐藏，例如 Codex 的 GPT-5.3-Codex-Spark：隐藏后卡片上和展开箭头下都不再显示，圆环、刘海岛、桌面卡片和菜单栏也不再跟随它，快用完的提醒同样不再发。每个服务商分别记住，切换界面语言后照样有效，点「全部显示」恢复；最后一项不能隐藏。停靠条圆环的右键菜单里也有这一项。本机接口 `/v1/limits` 仍返回全部额度。（[#2](https://github.com/gentpan/QuotaBar/issues/2)）
- Codex 卡片的「限额重置次数」下面新增「到期」一行，列出每次攒下的重置机会各自什么时候过期，最早的在前，例如「5 天 18 小时 · 18 天 23 小时 · 19 天 22 小时」；超过三次时显示其余次数。和额度行一样可以点击在倒计时和具体时间之间切换，悬停显示另一种写法。没有过期时间的不列出，停靠条圆环的详情里也有这一行。（[#3](https://github.com/gentpan/QuotaBar/issues/3)）
...
- DeepSeek 填好 API Key 后一直提示「服务商响应无法解析」、读不到余额：现在能正确显示账户余额，有赠送余额时分列充值和赠送；账户同时有人民币和美元余额时各显示一行，余额不足以调用 API 时会注明。（[#1](https://github.com/gentpan/QuotaBar/issues/1)）
- Grok 卡片上「每周额度」和「Grok Build」两条一模一样：本周额度只用在一个产品上时，那个产品的额度条和总额度数字、重置时间完全相同，现在不再重复显示；用在多个产品上时照常分项列出。（[#2](https://github.com/gentpan/QuotaBar/issues/2)）
- 设置里的输入框按 ⌘V 粘贴没有反应，粘贴 API Key 时看起来像输入框不接受输入：现在 ⌘V 粘贴、⌘C 拷贝、⌘X 剪切、⌘A 全选、⌘Z 撤销在设置和其他窗口里都能用，⌘W 关闭窗口。

</details>

<details>
<summary><b>2026-09-14</b> · 0.5.4 · 新增 2</summary>
...
<details>
<summary><b>2026-09-13</b> · 0.5.4 · 新增 9 · 样式 6 · 修复 3</summary>
